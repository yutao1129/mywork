...
async function GetCompletedProgressAndAmountByFactory(factory) {
    let querycompleted = {
        attributes: [
...
async function GetMaterialReceivingOutDate(MRProgress) {
    let queryMRDate = {
        attributes: [
            'receivingDate'
...
async function GetMaterialReceivingProgress() {
    let query = {
        attributes: [
            'order',
...
async function GetFabricInspectionProgress() {
    let query_fi = {
        attributes: [
...
async function GetProcessStationStatus() {
    let query = {
        attributes: [
            'order',
            'station',
    ...
    if (docs && docs.length > 0) {
        for (let item of docs) {
            ...
            orderResult.processStationStatus = item.station == null ? 0 : 1;
...
async function GetScheduleStatusByFactory(factory) {
    let queryschedule = {
        attributes: [
            //    'factory',
...
async function GetStepStatus(factory) {
    let queryPreceding = {
        attributes: [
            'amount',
            'cropEndDate',
...
exports.registerScheduleReportAPI = function (scheduleReportAPIRouter) {
    ...
    **/
    // scheduleReportAPIRouter.post('/scheduleReport/search', async (ctx) => {
    ...
    //         try {
    //             let ordersProcessAmount = await GetCompletedProgressAndTotalAmount();
    //             let ordersMaterialProgress = await GetMaterialReceivingProgress();
    //             let ordersProcessStationStatus = await GetProcessStationStatus();
    ...
    //                     console.log('ordersProcessAmount[order]', ordersProcessAmount[order]);
    ...
    //                         "materialProgress": ordersMaterialProgress[order] == undefined ? 0.0000 : ordersMaterialProgress[order].materialProgress,
    ...
    scheduleReportAPIRouter.post('/scheduleReport/searchByFactory', async (ctx) => {
        if (false === dbquery_1.checkRequestParamObject(ctx.request.body)) {
            ...
            try {
                ...
                let ordersProcessAmount = await GetCompletedProgressAndAmountByFactory(factory);
                //console.log('orderProcessAmount', ordersProcessAmount);

                let ordersMaterialProgress = await GetMaterialReceivingProgress();
                //console.log('ordersMaterialProgress', ordersMaterialProgress);

                let ordersMaterialOutDate = await GetMaterialReceivingOutDate();
                //console.log('ordersMaterialOutDate', ordersMaterialOutDate);

                let ordersFabricInspection = await GetFabricInspectionProgress();
                ...
                if (startdate == undefined||enddate==undefined) {
                    ctx.throw('db.invalidParameters:BE', 400);
                }
                let datespan = {[Sequelize_1.Op.between]: [startdate, enddate] };
                let queryreport = {
                    attributes: [
                        'id',
                        'orderID',
                ...
                if (docs && docs.length > 0) {
                    ...
                    for (let item of docs) {
                        var order = item.id;
                        var materialOutDate = ordersMaterialOutDate[order] == undefined || ordersMaterialOutDate[order] == 1 ? "" : "且超期"; //无收料记录时undefined
                        var materialProgress = ordersMaterialProgress[order] == undefined ? 0.0000 : (ordersMaterialProgress[order].materialProgress * 100).toFixed(0);
    ...
    scheduleReportAPIRouter.post('/scheduleReport/exceptionReport', async (ctx) => {
        if (false === dbquery_1.checkRequestParamObject(ctx.request.body)) {
            ...
            try {
                ...
                if (startdate == undefined||enddate==undefined) {
                    ctx.throw('db.invalidParameters:BF', 400);
                }
                ...
                if (docs && docs.length > 0) {
                    ...
                    for (let item of docs) {
                        ...
                        var record = {
                            ...
                            "cropProgress": ordersScheduleStatus[order] == undefined ? 0 : (ordersScheduleStatus[order].cropCompleteAmountSum / ordersScheduleStatus[order].amountSum).toFixed(4),
                            "stickProgress": ordersScheduleStatus[order] == undefined ? 0 : (ordersScheduleStatus[order].stickCompleteAmountSum / ordersScheduleStatus[order].amountSum).toFixed(4),
                            "sewingProgress": ordersScheduleStatus[order] == undefined ? 0 : (ordersScheduleStatus[order].sewingCompleteAmountSum / ordersScheduleStatus[order].amountSum).toFixed(4),
                            "lockProgress": ordersScheduleStatus[order] == undefined ? 0 : (ordersScheduleStatus[order].lockCompleteAmountSum / ordersScheduleStatus[order].amountSum).toFixed(4),
                            "ironProgress": ordersScheduleStatus[order] == undefined ? 0 : (ordersScheduleStatus[order].ironCompleteAmountSum / ordersScheduleStatus[order].amountSum).toFixed(4),
                            "packProgress": ordersScheduleStatus[order] == undefined ? 0 : (ordersScheduleStatus[order].packCompleteAmountSum / ordersScheduleStatus[order].amountSum).toFixed(4)
                        }
